package main

import (
	"log"
	"os"
	"testing"

	lambdat "gitlab.com/s-org-backend/models/lambda-testing"
)

type Period struct {
	StartTime string `json:"startTime"`
	EndTime   string `json:"endTime"`
	Subject   string `json:"subject"`
}

type ScheduleDay struct {
	Periods []Period `json:"periods"`
}

func TestMain(t *testing.T) {
	os.Setenv("RSAPRIVATEKEY", "")
	subjects := [...]string{"Math", "Math-SIP", "Deutsch", "Bio", "Geo", "Geo-ZIP"}
	schedule := [...]ScheduleDay{
		ScheduleDay{
			Periods: []Period{
				Period{StartTime: "07:30", EndTime: "8:10", Subject: "Kaka Emi"},
				Period{StartTime: "8:20", EndTime: "9:00", Subject: "Rusgench"},
				Period{StartTime: "9:10", EndTime: "9:50", Subject: "NE"},
				Period{StartTime: "10:10", EndTime: "10:50", Subject: "NE"},
			},
		},
		ScheduleDay{
			Periods: []Period{
				Period{StartTime: "07:30", EndTime: "8:10", Subject: "ZIP"},
				Period{StartTime: "8:20", EndTime: "9:00", Subject: "ZIP"},
				Period{StartTime: "9:10", EndTime: "9:50", Subject: "NE"},
				Period{StartTime: "10:10", EndTime: "10:50", Subject: "NE"},
			},
		},
		ScheduleDay{
			Periods: []Period{
				Period{StartTime: "07:30", EndTime: "8:10", Subject: "Bio"},
				Period{StartTime: "8:20", EndTime: "9:00", Subject: "Istoriq"},
				Period{StartTime: "9:10", EndTime: "9:50", Subject: "AE"},
				Period{StartTime: "10:10", EndTime: "10:50", Subject: "AE"},
			},
		},
	}
	body := map[string]interface{}{
		"username": "user3",
		"password": "secret12",
		"email":    "asdasda@abv.bg",
		"subjects": subjects,
		"schedule": schedule,
	}
	// log.Println(reflect.TypeOf(body["username"]).String() == "string")
	// err := emailx.Validate("test@abv.bg")
	err := sendEmail("martilevski1@abv.bg")
	log.Println(err, "err")
	return
	res, err := lambdat.InvokeHandler(handler, body)
	if err != nil {
		t.Error(err)
	}
	t.Log(res)
}
