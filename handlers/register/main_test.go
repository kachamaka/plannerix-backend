package main

import (
	"log"
	"net/http"
	"os"
	"testing"

	"github.com/shurcooL/vfsgen"
	lambdat "gitlab.com/zapochvam-ei-sq/plannerix-backend/models/lambda-testing"
)

// type Period struct {
// 	StartTime string `json:"startTime"`
// 	Duration  int    `json:"duration"`
// 	Subject   string `json:"subject"`
// }

// type ScheduleDay struct {
// 	Periods []Period `json:"periods"`
// }

func TestMain(t *testing.T) {
	os.Setenv("RSAPRIVATEKEY", "")
	// subjects := [...]string{"Math", "Math-SIP", "Deutsch", "Bio", "Geo", "Geo-ZIP"}
	// schedule := [...]ScheduleDay{
	// 	ScheduleDay{
	// 		Periods: []Period{
	// 			Period{StartTime: "07:30", Duration: 40, Subject: "Kaka Emi"},
	// 			Period{StartTime: "8:20", Duration: 40, Subject: "Rusgench"},
	// 			Period{StartTime: "9:10", Duration: 40, Subject: "NE"},
	// 			Period{StartTime: "10:10", Duration: 40, Subject: "NE"},
	// 		},
	// 	},
	// 	ScheduleDay{
	// 		Periods: []Period{
	// 			Period{StartTime: "07:30", Duration: 40, Subject: "ZIP"},
	// 			Period{StartTime: "8:20", Duration: 40, Subject: "ZIP"},
	// 			Period{StartTime: "9:10", Duration: 40, Subject: "NE"},
	// 			Period{StartTime: "10:10", Duration: 40, Subject: "NE"},
	// 		},
	// 	},
	// 	ScheduleDay{
	// 		Periods: []Period{
	// 			Period{StartTime: "07:30", Duration: 40, Subject: "Bio"},
	// 			Period{StartTime: "8:20", Duration: 40, Subject: "Istoriq"},
	// 			Period{StartTime: "9:10", Duration: 40, Subject: "AE"},
	// 			Period{StartTime: "10:10", Duration: 40, Subject: "AE"},
	// 		},
	// 	},
	// 	ScheduleDay{
	// 		Periods: []Period{
	// 			Period{StartTime: "07:30", Duration: 40, Subject: "Bio"},
	// 			Period{StartTime: "8:20", Duration: 40, Subject: "Istoriq"},
	// 			Period{StartTime: "9:10", Duration: 40, Subject: "AE"},
	// 			Period{StartTime: "10:10", Duration: 40, Subject: "AE"},
	// 		},
	// 	},
	// 	ScheduleDay{
	// 		Periods: []Period{
	// 			Period{StartTime: "07:30", Duration: 40, Subject: "Bio"},
	// 			Period{StartTime: "8:20", Duration: 40, Subject: "Istoriq"},
	// 			Period{StartTime: "9:10", Duration: 40, Subject: "AE"},
	// 			Period{StartTime: "10:10", Duration: 40, Subject: "AE"},
	// 		},
	// 	},
	// }
	body := map[string]interface{}{
		// "username": "testingWE26",
		// "password": "secret12",
		// "email":    "martilevski1@abv.bg",
		"verificationKey": "$2a$12$xClDFofHXAOx7H/Sd.1qieJvDjeIlmujefNj9kG9bIm2u43ZewqCO",
		// "email":    "traqn02@gmail.com",
		// "subjects": subjects,
		// "schedule": schedule,
	}
	// log.Println(reflect.TypeOf(body["username"]).String() == "string")
	// err := emailx.Validate("test@abv.bg")
	// err := sendEmail("martilevski1@abv.bg")
	// log.Println(err, "err")
	res, err := lambdat.InvokeHandler(handler, body)
	if err != nil {
		t.Error(err)
	}
	t.Log(res)
}

func TestPackageFunc(t *testing.T) {
	var fs http.FileSystem = http.Dir("./assets/")

	err := vfsgen.Generate(fs, vfsgen.Options{})
	if err != nil {
		log.Fatalln(err)
	}

}
